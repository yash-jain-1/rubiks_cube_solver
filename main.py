...
if len(scramble_seq) <= 4:
    ...
    print(
        "BFS Solution:",
        bfs_solution if bfs_solution else "No solution found within depth limit.",
    )
    astar_solution = astar_solver(cube.clone(), max_depth=10)
    print(
        "A* Solution:",
        astar_solution if astar_solution else "No solution found within depth limit.",
    )
else:
    print(
        "WARNING: BFS and A* solvers are too slow for real scrambles. Try with a simpler scramble (<=4 moves)."
    )
    sys.exit(0)
